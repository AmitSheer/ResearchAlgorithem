...
if __name__ == '__main__':
    ...
    for element in dataset['feed']['entry']:
        ...
        StartDate = None if dict(entry['d:StartDate']).keys().__contains__('@m:null') else datetime.strptime(
            entry['d:StartDate']['#text'], '%Y-%m-%dT%H:%M:%S')
        EndDate = None if dict(entry['d:EndDate']).keys().__contains__('@m:null') else datetime.strptime(
            entry['d:EndDate']['#text'], '%Y-%m-%dT%H:%M:%S')
        ReasonID = int(entry['d:ReasonID']['#text'])
        ReasonDesc = str(entry['d:ReasonDesc'])
        LastUpdatedDate = None if dict(entry['d:LastUpdatedDate']).keys().__contains__(
            '@m:null') else datetime.strptime(
            entry['d:LastUpdatedDate']['#text'], '%Y-%m-%dT%H:%M:%S')
        data = [BillHistoryInitiatorID, BillID, PersonID, IsInitiator, StartDate, EndDate, ReasonID, ReasonDesc,
                LastUpdatedDate]
        cursor.execute('''
            INSERT OR IGNORE INTO KNS_BillHistoryInitiator(BillHistoryInitiatorID, BillID, PersonID, IsInitiator, StartDate,
             EndDate, ReasonID, ReasonDesc, LastUpdateDate) VALUES (?,?,?,?,?,?,?,?,?)''',
                       data)
    cursor.execute('''
